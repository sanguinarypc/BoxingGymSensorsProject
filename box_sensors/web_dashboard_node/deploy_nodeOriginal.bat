...
echo scp -i "%KEY%" -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new dashboard.tgz %SERVER_USER%@%SERVER_IP%:%REMOTE_PATH%/ || (echo ERROR: scp failed & exit /b 1)
scp -P %SERVER_PORT% -i "%KEY%" -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new dashboard.tgz %SERVER_USER%@%SERVER_IP%:%REMOTE_PATH%/ || (echo ERROR: scp failed & exit /b 1)


echo.
echo [3/6] Running server-side deploy script...
echo ssh -i "%KEY%" -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new %SERVER_USER%@%SERVER_IP% "/usr/local/bin/dashboard-deploy-run" || (echo ERROR: remote deploy failed & exit /b 1)
ssh -p %SERVER_PORT% -i "%KEY%" -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new %SERVER_USER%@%SERVER_IP% "/usr/local/bin/dashboard-deploy-run" || (echo ERROR: remote deploy failed & exit /b 1)
